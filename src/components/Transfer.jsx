...
export default function Transfer({ listWallet }) {
  ...
  return (
    <div className="flex flex-col gap-4 p-4 border drop-shadow-sm rounded-md">
      ...
      <h1 className="text-2xl font-bold">Transfer Antar Wallet</h1>
      <div className="flex justify-between gap-2">
        <Dropdown
          color="blue"
          label={from ? from.title : "Dari Wallet"}
          dismissOnClick={true}
          style={{ width: "50%" }}
        >
          {listWallet.map((wallet) => (
            <Dropdown.Item
              key={wallet.properties.index}
              onClick={() => {
                setFrom(wallet.properties);
              }}
            >
      ...
      <Button color="blue" className="mt-2" onClick={simpan}>
        {isMutating ? (
          <svg
            aria-hidden="true"
